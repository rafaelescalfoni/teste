...
$(function(){
	loadFilmes();
	var ArrayFilmes = JSON.parse(data) ;

	$.each(ArrayFilmes, function(indice, filme){
		$(".thumbnail").append("<img/>").text(filme.figura);
		$(".caption").append("<h3/>").text(filme.titulo);
		$(".caption").append("<p/>").text(filme.resumo);
	});
});